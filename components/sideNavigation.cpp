...
public:
	SideNavigation(wxPanel* parent) : wxPanel(parent, ID_SIDE_NAVIGATION) {
		...
		//top_panel icons
		file_docker = new wxImagePanel(top_panel, icons_dir+"file-dock.png", wxBITMAP_TYPE_PNG, 0.0, 28);
		top_panel_sizer->Add(file_docker, 0, wxEXPAND | wxALL, 8);

		search = new wxImagePanel(top_panel, icons_dir+"search.png", wxBITMAP_TYPE_PNG, 0.0, 28);
		top_panel_sizer->Add(search, 0, wxEXPAND | wxALL, 8);

		wx = new wxImagePanel(top_panel, icons_dir+"wx.png", wxBITMAP_TYPE_PNG, 0.0, 28);
		top_panel_sizer->Add(wx, 0, wxEXPAND | wxALL, 8);

		extensions = new wxImagePanel(top_panel, icons_dir+"extensions.png", wxBITMAP_TYPE_PNG, 0.0, 28);
		...
		//bottom_panel icons
		settings = new wxImagePanel(bottom_panel, icons_dir+"settings.png", wxBITMAP_TYPE_PNG, 0.0, 28);
		bottom_panel_sizer->Add(settings, 0, wxEXPAND | wxALL, 6);

		hidde_sideNavigation = new wxImagePanel(bottom_panel, icons_dir+"hidde_sideNavigation.png", wxBITMAP_TYPE_PNG, 0.0, 28);
